...
def lambdaFile(fname1, fname2=""):
    ...
    lambda_dvdl1 = load.Col(fname1, 3)
    lambda_temp1 = load.Col(fname1, 4)
    lambda_vel1  = load.Col(fname1, 5)
    F_coulomb1   = load.Col(fname1, 6)
    F_corr1      = load.Col(fname1, 7)
    F_bias1      = load.Col(fname1, 8)
    F_ph1        = load.Col(fname1, 9)

    time2        = load.Col(fname2, 1)
    lambda_x2    = load.Col(fname2, 2)
    lambda_dvdl2 = load.Col(fname2, 3)
    lambda_temp2 = load.Col(fname2, 4)
    lambda_vel2  = load.Col(fname2, 5)
    F_coulomb2   = load.Col(fname2, 6)
    F_corr2      = load.Col(fname2, 7)
    F_bias2      = load.Col(fname2, 8)
    F_ph2        = load.Col(fname2, 9)

    fig.add_subplot(2, 4, 1)
    plt.plot(time1, lambda_x1, linewidth=0.5)
    if (fname2 != ""): plt.plot(time2, lambda_x2, linewidth=0.5)
    plt.title(r"$\lambda$-coordinate vs time")
    plt.xlabel("Time (ps)")
    plt.ylabel(r"$\lambda$-coordinate")
    plt.ylim(-0.1, 1.1)
    plt.ticklabel_format(axis='x', style='sci', scilimits=(0, 3))
    ...
    plt.plot(time1, lambda_temp1, linewidth=0.5, label="mean = {:.1f} (K)".format(sum(lambda_temp1)/len(lambda_temp1)))
    if (fname2 != ""): plt.plot(time2, lambda_temp2, linewidth=0.5, label="mean = {:.1f} (K)".format(sum(lambda_temp2)/len(lambda_temp2)))
    # meanRun = running_mean(lambda_temp)
    # plt.plot(time, meanRun, label="mean = {:.1f} K".format(meanRun[-1]))
    plt.title(r"$\lambda$-temperature vs time")
    plt.xlabel("Time (ps)")
    plt.ylabel(r"$\lambda$-temperature (K)")
    plt.ticklabel_format(axis='x', style='sci', scilimits=(0, 3))
    plt.legend()
    plt.grid()

    fig.add_subplot(2, 4, 3)
    plt.hist(lambda_vel1, density=True)
    if (fname2 != ""): plt.hist(lambda_vel2, density=True)
    plt.title(r"$\lambda$-velocity distribution")
    plt.xlabel(r"$\lambda$-velocity (km/s)")
    plt.ticklabel_format(axis='x', style='sci', scilimits=(0, 3))
    plt.grid()    
    ...
    plt.scatter(lambda_x1, F_corr1, s=10)
    ...
    plt.scatter(lambda_x1, F_bias1, s=10)
    ...
    plt.scatter(lambda_x1, F_ph1, s=10)
